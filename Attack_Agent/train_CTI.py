...
class Trainer(object):
    ...
    def generate_cluster(self, technique_id: str, hints: dict,
                          out_path: str, min_atoms: int = 10,
                          max_atoms: int = 20) -> str:
        """
        Sinh 1 subgraph cluster cho một technique cụ thể.

        Args:
            technique_id: MITRE technique ID (T1059, v.v.)
            hints: dict từ get_entity_constraints(technique_id)
            out_path: đường dẫn lưu file .dot output
            min_atoms: số node tối thiểu
            max_atoms: số node tối đa

        Returns: out_path nếu thành công, None nếu thất bại
        """
        print(f"[MaskGAF] Generating cluster for {technique_id} -> {out_path}")

        # Tạm override min/max atoms
        orig_min = self.args.min_atoms
        orig_max = self.args.max_atoms
        self.args.min_atoms = min_atoms
        self.args.max_atoms = max_atoms

        try:
            raw_txt = out_path.replace(".dot", "_raw.txt")
            result = self.generate_molecule(
                num=1,
                out_path=raw_txt,
                mute=True
            )
            ...
            self.args.min_atoms = orig_min
            self.args.max_atoms = orig_max

            if not os.path.exists(raw_txt):
                print(f"  WARNING: Raw output not found at {raw_txt}")
                return None

            # Convert raw text output sang .dot format
            from trans_gendata_CTI import convert_to_dot
            convert_to_dot(raw_txt, out_path, technique_hints=hints)
